...
class Installer {
	...
	public function modifyYaySMTPEmailLogs() {
		...
		if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table ) ) == $table ) {
			$check_col_1_exist = $wpdb->query( $wpdb->prepare( 'SHOW COLUMNS FROM %s LIKE %s ', $table, 'root_name' ) );
			...
			$check_col_2_exist = $wpdb->query( $wpdb->prepare( 'SHOW COLUMNS FROM %s LIKE %s ', $table, 'extra_info' ) );

...
class PluginCore {
	...
	public function doSmtperInit( $obj ) {
		...
		if ( $useFallbackSmtp ) {
			...
			$obj->SMTPSecure = ! empty( $settings['fallback_auth_type'] ) && 'ssl' === $settings['fallback_auth_type'] ? 'ssl' : 'tls';
			...
			if ( 'smtp' == $currentMailer ) {
				...
				$obj->SMTPSecure = ! empty( $smtpSettings['encryption'] ) && 'ssl' === $smtpSettings['encryption'] ? 'ssl' : 'tls';

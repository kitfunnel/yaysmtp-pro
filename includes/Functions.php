		...
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				if ( ! empty( $valSearch ) ) {
					...
					$sqlRepareAll = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}yaysmtp_email_logs WHERE $whereQuery ORDER BY %s %s", $sortField, $sortVal );
					$sqlRepare    = $wpdb->prepare(
						"SELECT * FROM {$wpdb->prefix}yaysmtp_email_logs WHERE $whereQuery ORDER BY %s %s LIMIT %d OFFSET %d",
						$sortField,
						$sortVal,
						$limit,
						$offset
	...
	public function deleteEmailLogs() {
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				$deleted = $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}yaysmtp_email_logs WHERE ID IN( $ids )" ) );
